...
const SharedTransition = (
  ...
  const animation = useRef(mainAnimationValue || new Animated.Value(0)).current;
  const scale = useRef(scaleAnimationValue || new Animated.Value(1)).current;
  ...
  const [
    ...
    gestureState,
    closeButtonState,
  ] = useValues([1, 0, 0, 0, 1, State.UNDETERMINED, State.UNDETERMINED], []);
